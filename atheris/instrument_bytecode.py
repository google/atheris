...
Mainly the function patch_code(), which can instrument a code object
and the helper class Instrumentor.
"""
...
class Instruction:
    ...
    def get_stack_effect(self):
        ...
        return dis.stack_effect(self.opcode, (self.arg if self.has_argument() else None))
    ...
    def adjust(self, changed_offset, size, keep_ref):
        ...
        (2) Signal this instruction that an instruction at offset `changed_offset` - 0.5
            has increased in size. If `changed_offset` is self.offset + 0.5, this
            instruction increased in size.
        
        Either way, adjust the current offset, reference and argument accordingly.
        """
...
class BasicBlock:
    def __init__(self, instructions, last_one):
        ...
        if last_one or last_instr.mnemonic in ENDS_FUNCTION:
            ...
            self.edges = list(set([ last_instr.reference, last_instr.offset + last_instr.get_size() ]))
        else:
...
class Instrumentor:
    ...
    How to insert code:
        ...
        5. Insert your instruction list into the instruction list of the basic block
        6. Call _handle_size_changes()
    Take a look at trace_control_flow() and trace_data_flow() for examples.
    
    Note that this calls only supports insertions, not deletions.
    """
    ...
    def _build_cfg(self):
        ...
        for instruction in dis.get_instructions(self._code):
            ...
            if instruction.opname == "EXTENDED_ARG":
                ...
                instr_list.append(Instruction(lineno, offset, instruction.opcode, (arg << 8) | instruction.arg, min_size=length))
                arg = None
                offset = None
                length = Instruction.get_fixed_size()
                
            else:
                instr_list.append(Instruction(lineno, instruction.offset, instruction.opcode, instruction.arg))
        ...
        for i in range(len(basic_block_borders) - 1):
            ...
            bb = BasicBlock(instr_list[start_of_bb:end_of_bb], i == len(basic_block_borders) - 2)
            self._cfg[bb.id] = bb
    ...
    def _check_state(self):
        ...
        while i < len(listing) - 1:
            assert(listing[i].offset + listing[i].get_size() == listing[i + 1].offset)
        ...
        while True:
            ...
            while i < len(listing) - 1:
                ...
                if next_offset < listing[i + 1].offset:
                    raise Exception(f"Something weird happened with the offsets at offset {listing[i].offset}")
    ...
    def _generate_loc_invocation(self, lineno, offset):
        ...
        to_insert.append(Instruction(lineno, offset, dis.opmap["LOAD_GLOBAL"], name_module))
        offset += to_insert[-1].get_size()
        to_insert.append(Instruction(lineno, offset, dis.opmap["LOAD_ATTR"], name_cov))
        offset += to_insert[-1].get_size()
        to_insert.append(Instruction(lineno, offset, dis.opmap["LOAD_CONST"], self._get_counter()))   
        offset += to_insert[-1].get_size()
    ...
    def _generate_cmp_invocation(self, op, lineno, offset):
        ...
        to_insert.append(Instruction(lineno, offset, dis.opmap["LOAD_GLOBAL"], name_module))
        offset += to_insert[-1].get_size()
        to_insert.append(Instruction(lineno, offset, dis.opmap["LOAD_ATTR"], name_cmp))
        offset += to_insert[-1].get_size()
        to_insert.append(Instruction(lineno, offset, dis.opmap["ROT_THREE"]))
        offset += to_insert[-1].get_size()
        to_insert.append(Instruction(lineno, offset, dis.opmap["LOAD_CONST"], const_op))
        offset += to_insert[-1].get_size()
        to_insert.append(Instruction(lineno, offset, dis.opmap["LOAD_CONST"], const_pc))
        offset += to_insert[-1].get_size()
        to_insert.append(Instruction(lineno, offset, dis.opmap["LOAD_CONST"], const_False))
        offset += to_insert[-1].get_size()
    ...
    def _generate_const_cmp_invocation(self, op, lineno, offset, switch):
        ...
        to_insert.append(Instruction(lineno, offset, dis.opmap["LOAD_CONST"], const_pc))
        offset += to_insert[-1].get_size()
        to_insert.append(Instruction(lineno, offset, dis.opmap["LOAD_CONST"], const_True))
        offset += to_insert[-1].get_size()
        to_insert.append(Instruction(lineno, offset, dis.opmap["CALL_FUNCTION"], 5))
        offset += to_insert[-1].get_size()
    ...
    def insert_registration(self, num_counters):
        ...
        to_insert.append( Instruction(lineno, offset, dis.opmap["LOAD_CONST"], const_0) )
        offset += to_insert[-1].get_size()
        to_insert.append( Instruction(lineno, offset, dis.opmap["LOAD_CONST"], const_None) )
        offset += to_insert[-1].get_size()
        to_insert.append( Instruction(lineno, offset, dis.opmap["IMPORT_NAME"], name_module) )
        offset += to_insert[-1].get_size()
        to_insert.append( Instruction(lineno, offset, dis.opmap["STORE_GLOBAL"], name_module) )
        offset += to_insert[-1].get_size()
        
        # Insert a call to the registration function
        to_insert.append( Instruction(lineno, offset, dis.opmap["LOAD_GLOBAL"], name_module) )
        offset += to_insert[-1].get_size()
        to_insert.append( Instruction(lineno, offset, dis.opmap["LOAD_ATTR"], name_reg) )
        offset += to_insert[-1].get_size()
        to_insert.append( Instruction(lineno, offset, dis.opmap["LOAD_CONST"], const_num_counters) )
        offset += to_insert[-1].get_size()
        to_insert.append( Instruction(lineno, offset, dis.opmap["CALL_FUNCTION"], 1) )
        offset += to_insert[-1].get_size()
        to_insert.append( Instruction(lineno, offset, dis.opmap["POP_TOP"]) )
                ...
                if instr.mnemonic == "LOAD_CONST":
                    ...
                    if not (tos_is_constant and tos1_is_constant):
                        ...
                        if (not tos_is_constant) and (not tos1_is_constant):
                            total_size, to_insert = self._generate_cmp_invocation(instr.arg, instr.lineno, offset)
                        
                        # One item is constant, one is non-constant
                        else:
                            total_size, to_insert = self._generate_const_cmp_invocation(instr.arg, instr.lineno, offset, tos_is_constant)
...
def patch_code(code, trace_dataflow, nested=False):
    ...
    for i in range(len(inst.consts)):
        if isinstance(inst.consts[i], types.CodeType):
            if (inst.consts[i].co_name in ["<lambda>", "<module>" if not nested else None] 
